namespace BetterDrops_CursedMod.Features.Components
{
    ...
    public class DropController : MonoBehaviour
    {
        ...
        {
            CursedLightSource l = CursedLightSource.Create(null, null, null);
            l.LightColor = _color;
        ...
        {
            CursedPrimitiveObject p = CursedPrimitiveObject.Create(type, null, null, scale, color);
            Transform t = p.Base.transform;
            t.SetParent(parent);
            t.localPosition = pos;
            t.localRotation = Quaternion.Euler(rot);
            p.MovementSmoothing = 60;
            p.Spawn(t.position, t.eulerAngles, scale);
        }
        ...
        private void OnCollisionEnter()
        {
            ...
            DeployBalloon();
            AddTrigger();
            if (Plugin.Instance.Config.AutoOpen > 0)
                Invoke(nameof(OpenDrop), Plugin.Instance.Config.AutoOpen);
        }
        ...
        private void OpenDrop()
        {
            ...
            {
                var item = CursedServer.LocalPlayer.ReferenceHub.inventory.ServerAddItem(itemType);
